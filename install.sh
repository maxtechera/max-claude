...
# SoloClaude Installer Script
# Installs SoloClaude configuration framework to enhance Claude Code
# Version: 2.0.0
# License: MIT
# Repository: https://github.com/NomenAK/SoloClaude
...
readonly CONFIG_FILE=".soloclaude.conf"
...
# Returns: None
show_usage() {
    echo "SoloClaude Installer v$SCRIPT_VERSION"
    echo ""
    ...
    echo "  --uninstall          Remove SoloClaude from specified directory"
    ...
    echo "  --check-update       Check for SoloClaude updates"
    ...
    echo "  $0 --uninstall              # Remove SoloClaude"
...
# Description: Check for SoloClaude updates from GitHub
# Parameters: None
# Returns: 0 if update available, 1 if up to date, 2 on error
check_for_updates() {
    local repo_url="https://api.github.com/repos/nshkrdotcom/SoloClaude/releases/latest"
    ...
    log "Checking for SoloClaude updates..."
    ...
    if compare_versions "$SCRIPT_VERSION" "$latest_version"; then
        ...
        echo "Download: https://github.com/nshkrdotcom/SoloClaude/releases/latest"
        return 0
    else
...
load_default_config() {
    # System-wide config
    if [[ -f "/etc/soloclaude.conf" ]]; then
        load_config "/etc/soloclaude.conf"
    fi
    
    # User config
    if [[ -f "$HOME/.soloclaude.conf" ]]; then
        load_config "$HOME/.soloclaude.conf"
    fi
    
    # Local config
    if [[ -f ".soloclaude.conf" ]]; then
        load_config ".soloclaude.conf"
    fi
}
...
if [[ "$UNINSTALL_MODE" = true ]]; then
    echo -e "${GREEN}SoloClaude Uninstaller${NC}"
    echo "========================"
    echo -e "Target directory: ${YELLOW}$INSTALL_DIR${NC}"
    echo ""
    
    if [[ ! -d "$INSTALL_DIR" ]]; then
        echo -e "${RED}Error: SoloClaude not found at $INSTALL_DIR${NC}"
        exit 1
    fi
    
    if [[ "$FORCE_INSTALL" != true ]]; then
        echo -e "${YELLOW}This will remove SoloClaude from $INSTALL_DIR${NC}"
    ...
    echo "Removing SoloClaude files while preserving user data..."
    
    # Remove SoloClaude files but preserve user data
    removed_count=0
    preserved_count=0
    
    # First, remove all SoloClaude files (those that would be installed)
    if [[ -d "$INSTALL_DIR" ]]; then
        ...
        while IFS= read -r installed_file; do
            ...
            if is_preserve_file "$installed_file"; then
                ...
                # Check if this file exists in source (is a SoloClaude file)
        ...
        if [[ -z "$(ls -A "$INSTALL_DIR" 2>/dev/null)" ]]; then
            ...
            echo -e "${GREEN}✓ SoloClaude uninstalled completely!${NC}"
        else
            echo ""
            echo -e "${GREEN}✓ SoloClaude uninstalled successfully!${NC}"
...
if [[ "$VERIFY_MODE" = true ]]; then
    echo -e "${GREEN}SoloClaude Verification${NC}"
    echo "========================="
    echo -e "Target directory: ${YELLOW}$INSTALL_DIR${NC}"
    echo ""
    ...
    # Check if we're in SoloClaude directory
    if [ ! -f "CLAUDE.md" ] || [ ! -d ".claude" ] || [ ! -d ".claude/commands" ]; then
        echo -e "${RED}Error: This script must be run from the SoloClaude directory${NC}"
        echo ""
        echo "Expected files not found. Please ensure you are in the root SoloClaude directory."
        echo "Missing: $([ ! -f "CLAUDE.md" ] && echo "CLAUDE.md ")$([ ! -d ".claude" ] && echo ".claude/ ")$([ ! -d ".claude/commands" ] && echo ".claude/commands/")"
        echo ""
        echo "Solution: cd to the SoloClaude directory and run: ./install.sh --verify-checksums"
        exit 1
    fi
...
echo -e "${GREEN}SoloClaude Installer${NC}"
echo "======================"
...
if [[ "$FORCE_INSTALL" != true ]]; then
    if [[ "$UPDATE_MODE" = true ]]; then
        echo -e "${YELLOW}This will update SoloClaude in $INSTALL_DIR${NC}"
    else
        echo -e "${YELLOW}This will install SoloClaude in $INSTALL_DIR${NC}"
    fi
    echo -n "Are you sure you want to continue? (y/n): "
...
if [ ! -f "CLAUDE.md" ] || [ ! -d ".claude" ] || [ ! -d ".claude/commands" ]; then
    ...
    echo "Missing: $([ ! -f "CLAUDE.md" ] && echo "CLAUDE.md ")$([ ! -d ".claude" ] && echo ".claude/ ")$([ ! -d ".claude/commands" ] && echo ".claude/commands/")"
    echo ""
    echo "Solution: cd to the SoloClaude directory and run: ./install.sh"
    exit 1
fi
...
log_verbose "SoloClaude version: $SUPERCLAUDE_VERSION"
...
if [ -d "$INSTALL_DIR" ] && [ "$(ls -A "$INSTALL_DIR" 2>/dev/null)" ]; then
    ...
    if [ "$backup_choice" = "y" ]; then
        ...
        BACKUP_DIR="$(dirname "$INSTALL_DIR")/soloclaude-backup.${backup_timestamp}.${backup_random}"
        ...
        find . -mindepth 1 -maxdepth 1 \( -name "soloclaude-backup.*" -prune \) -o -print0 | \
...
echo ""
if [[ "$UPDATE_MODE" = true ]]; then
    echo "Updating SoloClaude..."
else
    echo "Installing SoloClaude..."
fi
...
if command -v sha256sum &> /dev/null && [[ "$DRY_RUN" != true ]]; then
    ...
    done < <(find . -type f -not -path "./soloclaude-backup.*" -not -name ".checksums" | sort)
...
if [ "$actual_files" -ge "$expected_files" ] && [ "$critical_files_ok" = true ] && [ $VERIFICATION_FAILURES -eq 0 ]; then
    ...
    echo ""
    if [[ "$UPDATE_MODE" = true ]]; then
        echo -e "${GREEN}✓ SoloClaude updated successfully!${NC}"
        echo ""
        # Check for .new files
        ...
        echo -e "${GREEN}✓ SoloClaude installed successfully!${NC}"
        echo ""
        echo "Next steps:"
